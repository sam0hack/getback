...
echo "Getback your autoremove packages\n " . PHP_EOL;
...
echo  "Default history.log location is ( /var/log/apt/history.log ) is selected\n\n" . PHP_EOL;
...

{
			...
			$rd= new readfile(); //Read the file
			$result=$rd->searchdate();//Get Result

			if($result=='X') //X means autoremove not found in the history/log file
			{
			echo "autoremove not found in your system".PHP_EOL;
			}
	else
	{
	echo "Packages found \n" . PHP_EOL;
	...
	echo "Do you want to countinue yes OR no\n" . PHP_EOL;  
	$sys1=read_stdin();   //Read the user input                      

	if($sys1=='yes' || $sys1=='y')
	{


	echo "Enter architecture amd64 or x86 or i386: \n" . PHP_EOL;
	$sys=read_stdin();



	$obj= new readfile();
	$string=$obj->searchdate();




	$t2= preg_replace("/\([^)]+\)/","",$string); 





	$str = trim(preg_replace('/\s*\([^)]*\)/', '', $string)); //replace unused chars

	$bc=str_replace (":$sys" ,"" , $str); 
	$ch=str_replace ("," ," " , $bc);            

	$mc=str_replace ("Remove:" ," " , $ch);

	$aptget="sudo apt-get install " . $mc; //Concat sudo apt-get install with filtered packages list
	//$output = shell_exec($aptget);  #Terminal execute here

	//echo $output. "\n\n " . PHP_EOL; //Show Output packages
	echo  "Execution has been Done :)\n\n " . PHP_EOL; //Final message
	}
			
			else

			{
			echo "Execution has been Stoped\n" . PHP_EOL;
			exit();
